...
public class MedicalHistoryTab {
	...
	public void editMedicalHistory() throws IOException, SQLException {
		...
		for(int excelrow=1;excelrow<=rowcount;excelrow++) {
			...
			String systemname=ExcelRead.getCellData(path, sheet, excelrow, 3);
			String diseasename=ExcelRead.getCellData(path, sheet, excelrow, 4);
			String note=ExcelRead.getCellData(path, sheet, excelrow, 5);
			...
			con.clickOnMedicalHistoryTab();
			logger.info("Clicked On Medical History tab");
			Util.sleepTime(1000);
			
			con.expandSystemName(systemname);
			logger.info("Expande system  :"+systemname);
			Util.sleepTime(1000);

			con.clickOnDiseaseEditBtn(diseasename);
			logger.info("Clicked On Disease edit button  :"+diseasename);
			Util.sleepTime(1000);

			if(note!="") {

				con.enterNotes(note);
				logger.info("Entered Reason  :"+note);
				Util.sleepTime(500);
			}

			String updatebtnstatus=con.getUpdateBtnStatus();
			if(!updatebtnstatus.contains("disabled")) {
				...
				ExcelRead.setCellData(path, sheet, excelrow, 6, flag);
				Util.sleepTime(1000);

			}else {
				
				ExcelRead.setCellData(path, sheet, excelrow, 6, "Update button is disabled");
				Util.sleepTime(1000);
				
				con.clickOnCancelBtn1();
